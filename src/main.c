...
int
handle_show (int argc, char *argv[])
{
  int show_all = 0;
  int filter_set = 0;
  char key[256];
 ...
 */
int
handle_edit (int argc, char *argv[])
{
  char key[256];
  snprintf (key, sizeof (key), "%s", KEY);
...

  {
    ...
    if (attribute)
      fwrite (attribute, 1, strlen (attribute), fp);
  }
  ...
  fread (edit, 1, length, fp_read); // TODO error handling
              ...
              if (removexattr (file, key) < 0)
                {
                  if (errno == ENODATA)
                    {
                      fprintf (stderr,
                               "%s rm: attribute '%s' does not exist\n",
                               argv[0], key);
                    }
                  else
                    {
...
int
handle_help (int argc, char *argv[])
{
  if (argc != 3)
    {
      print_usage (argv[0]);
  ...
  switch (parse_command (argv[2]))
    ...
    case CMD_SHOW:
      ...
      break;
    case CMD_EDIT:
      print_edit_help (argv[0]);
      break;
    case CMD_RM:
      print_rm_help (argv[0]);
      break;
    case CMD_HELP:
      print_help_help (argv[0]);
      break;
    case CMD_UNKNOWN:
      printf ("%s help: unknown command '%s'\n", argv[0], argv[2]);
      return -1;
    }
  return 0;
}
...
main (int argc, char *argv[])
{
  ...
  switch (parse_command (argv[1]))
    {
    case CMD_ADD:
      if (handle_add (argc, argv) < 0)
        exit (EXIT_FAILURE);
      break;
    case CMD_SHOW:
      if (handle_show (argc, argv) < 0)
        exit (EXIT_FAILURE);
      break;
    case CMD_EDIT:
      if (handle_edit (argc, argv) < 0)
        exit (EXIT_FAILURE);
      break;
    case CMD_RM:
      if (handle_rm (argc, argv) < 0)
        exit (EXIT_FAILURE);
      break;
    case CMD_HELP:
      if (handle_help (argc, argv) < 0)
        exit (EXIT_FAILURE);
      break;
    case CMD_UNKNOWN:
